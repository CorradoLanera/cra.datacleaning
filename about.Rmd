...
1. **Rivedere** 
  - La distinzione tra le tipologie di dati in cui ci si può imbattere nella ricerca clinica
  - I principali tipi di dati gestiti da R, inclusi per i dati mancanti
  

2. **Imparare** 
  - Definire le caratteristiche di un dataset finalizzato all'analisi di dati (cosiddetto __tidy__)
  - Cercare, estrarre, rimuovere informazioni strutturate, definite nei dati tramite testo
  - Manipolare e trasformare basi di dati globalmente in forma o localmente nel contenuto
  - Risolvere i problemi più comuni in cui ci si imbatte quando si importano dati raccolti su fogli di calcolo (Excel)

3. **Applicare** 
  - il pacchetto `{readr}` (e `{readxl}`) per importare i dati
  - il pacchetto `{janitor}` per uniformare i metadati principali di una base di dati (nomi delle variabili) e rimuovere righe/colonne prive di contenuto
  - il pacchetto `{tidyr}` e `{dplyr}` per manipolarli rispettivamente globalmente e localmente
  - il pacchetto `{stringr}` e le espressioni regolari per gestire informazioni codificate dentro al testo
  - il pacchetto `{unheadr}` per risolvere problemi comuni di importazione dati dai fogli di calcolo


### In parole povere...

Imparerai a importare correttamente una base di dati dal mondo esterno dividendo, aggregando, estraendo, trasformando, lei stessa o il suo contenuto fino ad arrivare a forma, coerenza e completezza utili alle analisi; il tutto, operando solo tramite codice (R), riproducibile e senza (possibilmente) alcuna manipolazione diretta del contenuto del file sorgente da parte tua.
